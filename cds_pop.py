...
with open(os.path.join(path['in'], 'chr_{}.fasta'.format(ch)), 'r') as f:
    for record in SeqIO.parse(f, 'fasta'):
        ...
        if 'ZI103_Chr2L' in des:
            seq_ref = record.seq
        elif 'ref' in des.lower():
            pass
        else:
            seq[des] = record.seq
...
for k, v in stat.items():
    if k in ['sample']:
        ...
        pd.Series(v).to_csv(os.path.join(path['out'], 'cds_pop_{}.csv'.format(k)))

t2 = time.time()

with open(os.path.join(path['out'], 'runtime_cds_pop.csv'), 'w') as f:
